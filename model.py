...
from util.image_utils import create_gaussian_heatmap_template, create_dist_mat, \
	preprocess_image_for_segmentation, pad, resize
from util.dataloader import SceneDataset, scene_collate
	...
	def __init__(self, obs_len, pred_len, segmentation_model_fp, use_features_only=False, semantic_classes=6,
		...
		if segmentation_model_fp is not None:
			...
			if use_features_only:
				...
				semantic_classes = 6  # instead of classes use number of feature_dim(16)
		else:
...
class YNet:
    def __init__(self, obs_len, pred_len, params):
        ...
        self.model = YNetTorch(obs_len=obs_len,
							   ...
							   encoder_channels=params['encoder_channels'], # params['encoder_channels']
							   decoder_channels=params['decoder_channels'],
							   waypoints=1) # len(params['waypoints'])
...
# 										waypoints=1, resize=params['resize'], # params['waypoints']
