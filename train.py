...
def train(model, train_loader, train_images, e, obs_len, pred_len, batch_size, params, gt_template, device, epoch, input_template, optimizer, criterion): # , dataset_name, homo_mat
    """
...
#                 break
        ...
        for i in range(0, len(trajectory), batch_size):
            ...
            import numpy as np
            # import cv2
            # image = np.array(train_images[scene].permute(1, 2, 0))[:, :, [2, 1, 0]] # 210 120 021 012 201 102
            # # cv2.imshow('My Image', image)
            # plt.imshow(image)
            # plt.show()
            
            # feature_map = np.array(gt_future_map.cpu().reshape(1,320,256).permute(1, 2, 0))
            # goal_map = np.array(pred_goal_map.cpu().reshape(1,320,256).permute(1, 2, 0))
            # plt.imshow(feature_map)
            # plt.imshow(goal_map)
            # plt.show()
            # print('train_images[scene]',train_images[scene].shape,type(train_images[scene]))
            # print('observed',observed.shape,type(observed))
            # print('observed_map',observed_map.shape,type(observed_map))
            # print('semantic_map',semantic_map.shape,type(semantic_map))
            # print('gt_future', gt_future.shape,type(gt_future))
            # print('gt_waypoint_map',gt_waypoint_map.shape,type(gt_waypoint_map))
            # print('traj_input',traj_input[0].shape,type(traj_input)) #list
            # print('feature_input',feature_input.shape,type(feature_input))
            # print('gt_future_map',gt_future_map.shape,type(gt_future_map))
            # print('pred_goal_map',pred_goal_map.shape,type(pred_goal_map))
			# Backprop
            ...
            with torch.no_grad():
                ...
                # import cv2
                # image = np.array(train_images[scene].permute(1, 2, 0))[:, :, [0, 0, 0]] # 210 120 021 012 201 102 [:, :, [0, 2, 1]]
                # # cv2.imshow('My Image', image)
                # plt.imshow(image)
                # tr = pred_traj.tolist()
                # gt = gt_future.tolist()
